    ...
    if show_all:
        ...
        for i, f in enumerate(pivot['filters']):
            ...
            try:
                ...
                # Silently ignore all errors in filter UI
                continue
                    ...
                    if op == "is between (inclusive)" and is_date_op:
                        ...
                        if op == "is current month":
                            ...
                            if not pd.isna(val_dt):
                                filtered_df = filtered_df[col_data_dt == val_dt]
                    else:
        ...
        def generate_python_code():
            ...
            code_lines.append("# 1. Load your DataFrame (replace with your actual data loading)")
            code_lines.append("# Example: df_original = pd.read_csv('your_file.csv')")
            code_lines.append("df_original = pd.DataFrame() # Placeholder - ensure this is your actual DataFrame\n")
            code_lines.append("df_filtered = df_original.copy()\n")
            code_lines.append("# 2. Apply Filters")
            if not pivot['filters']:
                code_lines.append("# No filters applied.")
            active_filters = [f_c for f_c in pivot['filters'] if f_c["column"] and (f_c["value"] is not None or isinstance(f_c["value"], list) or f_c["operator"] in ["is current month", "is previous month", "is next month", "is current year", "is previous year", "is next year"])]
            for i, f_config in enumerate(active_filters):
                col = f_config["column"]
                op = f_config["operator"]
                val_config = f_config["value"]
                is_date_op_code_gen = op in ["is exactly", "is not", "is after", "is on or after",
                                     "is before", "is on or before", "is between (inclusive)",
                                     "is current month", "is previous month", "is next month",
                                     "is current year", "is previous year", "is next year"]
                if is_date_op_code_gen:
                    code_lines.append(f"df_filtered['{col}'] = pd.to_datetime(df_filtered['{col}'], errors='coerce')")
                    code_lines.append(f"df_filtered = df_filtered[df_filtered['{col}'].notna()] # Remove rows where date conversion failed")
                    if op == "is between (inclusive)":
                        start_date_str = pd.to_datetime(val_config[0]).strftime('%Y-%m-%d')
                        end_date_str = pd.to_datetime(val_config[1]).strftime('%Y-%m-%d')
                        code_lines.append(f"start_date_{i} = pd.to_datetime('{start_date_str}')")
                        code_lines.append(f"end_date_{i} = pd.to_datetime('{end_date_str}')")
                        code_lines.append(f"df_filtered = df_filtered[df_filtered['{col}'].between(start_date_{i}, end_date_{i}, inclusive='both')]")
                    elif op in ["is current month", "is previous month", "is next month", "is current year", "is previous year", "is next year"]:
                        code_lines.append(f"today_{i} = pd.Timestamp.now().normalize()")
                        if op == "is current month":
                            code_lines.append(f"df_filtered = df_filtered[(df_filtered['{col}'].dt.year == today_{i}.year) & (df_filtered['{col}'].dt.month == today_{i}.month)]")
                        elif op == "is previous month":
                            code_lines.append(f"current_month_start_{i} = today_{i}.replace(day=1)")
                            code_lines.append(f"prev_month_end_{i} = current_month_start_{i} - timedelta(days=1)")
                            code_lines.append(f"prev_month_start_{i} = prev_month_end_{i}.replace(day=1)")
                            code_lines.append(f"df_filtered = df_filtered[df_filtered['{col}'].between(prev_month_start_{i}, prev_month_end_{i}, inclusive='both')]")
                        elif op == "is next month":
                            code_lines.append(f"current_month_end_{i} = today_{i} + pd.offsets.MonthEnd(0)")
                            code_lines.append(f"next_month_start_{i} = current_month_end_{i} + timedelta(days=1)")
                            code_lines.append(f"next_month_end_{i} = next_month_start_{i} + pd.offsets.MonthEnd(0)")
                            code_lines.append(f"df_filtered = df_filtered[df_filtered['{col}'].between(next_month_start_{i}, next_month_end_{i}, inclusive='both')]")
                        elif op == "is current year":
                            code_lines.append(f"df_filtered = df_filtered[df_filtered['{col}'].dt.year == today_{i}.year]")
                        elif op == "is previous year":
                            code_lines.append(f"df_filtered = df_filtered[df_filtered['{col}'].dt.year == today_{i}.year - 1]")
                        elif op == "is next year":
                            code_lines.append(f"df_filtered = df_filtered[df_filtered['{col}'].dt.year == today_{i}.year + 1]")
                    else:
                        val_dt = pd.to_datetime(val_config, errors='coerce')
                        if not pd.isna(val_dt):
                            code_lines.append(f"val_dt_{i} = pd.to_datetime('{val_dt.strftime('%Y-%m-%d')}')")
                            operator_map = {"is exactly": "==", "is not": "!=", "is after": ">", "is on or after": ">=", "is before": "<", "is on or before": "<="}
                            code_lines.append(f"df_filtered = df_filtered[df_filtered['{col}'] {operator_map[op]} val_dt_{i}]")
                elif op in ["contains", "does not contain"]:
                    negation = "~" if op == "does not contain" else ""
                    code_lines.append(f"df_filtered = df_filtered[{negation}df_filtered['{col}'].astype(str).str.contains(r'''{str(val_config)}''', case=False, na=False)]")
                else:
                    val_repr = f"'{str(val_config)}'" if isinstance(val_config, str) else str(val_config)
                    code_lines.append(f"df_filtered = df_filtered[df_filtered['{col}'] {op} {val_repr}]")
            code_lines.append("\n# 3. Create Pivot Table")
            value_cols_for_pivot_code = [item["value_col"] for item in pivot['value_agg_list'] if item["value_col"]]
            aggfunc_dict_code = {item["value_col"]: item["agg_func"] for item in pivot['value_agg_list'] if item["value_col"]}
            code_lines.append("pivot_params = {")
            code_lines.append(f"    'index': {pivot['index_cols'] if pivot['index_cols'] else None},")
            code_lines.append(f"    'columns': {pivot['column_cols'] if pivot['column_cols'] else None},")
            code_lines.append(f"    'values': {value_cols_for_pivot_code if value_cols_for_pivot_code else None},")
            code_lines.append(f"    'aggfunc': {aggfunc_dict_code if aggfunc_dict_code else 'size'},")
            if pivot['fill_value_enabled']:
                fill_val_repr = f"'{pivot['custom_fill_value']}'" if isinstance(pivot['custom_fill_value'], str) else pivot['custom_fill_value']
                code_lines.append(f"    'fill_value': {fill_val_repr},")
            if pivot['margins_enabled']:
                code_lines.append(f"    'margins': True,")
                code_lines.append(f"    'margins_name': '{pivot['margins_name']}',")
            code_lines.append("}\n")
            code_lines.append("# Remove None params for pandas pivot_table call if they were None initially")
            code_lines.append("pivot_params = {k: v for k, v in pivot_params.items() if not (k in ['index', 'columns', 'values'] and v is None)}\n")
            code_lines.append("try:")
            code_lines.append("    pivot_df = pd.pivot_table(df_filtered, **pivot_params)")
            code_lines.append("    print('Pivot table created successfully.')")
            code_lines.append("    # print(pivot_df)")
            code_lines.append("except Exception as e:")
            code_lines.append("    print(f'Error creating pivot table: {e}')\n")
            pivot['generated_code'] = "\n".join(code_lines)
            pivot['last_success'] = "Python code generated!"
            pivot['last_error'] = None
        if st.sidebar.button("Generate Python Code", key=f"gen_code_{st.session_state.active_pivot}"):
            generate_python_code()
        # --- Display Messages ---
        # Remove all error/success messages from the main area
        # --- Display Pivot Table ---
        if pivot['pivot_df'] is not None:
